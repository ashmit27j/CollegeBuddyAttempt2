...
import { Loader, UserX, Mail, PhoneCall } from "lucide-react";
	...
	return (
		...
		<div className="flex flex-col h-screen bg-gray-100 bg-opacity-50">
			...
			<div className="flex-grow flex flex-col p-4 md:p-6 lg:p-8 overflow-hidden max-w-7xl mx-auto w-full">
				<div className="flex items-center justify-between mb-4 pr-4 bg-white rounded-2xl shadow p-3">
					...
					<div className="flex gap-3 pr-3">
						<a href={`mailto:${match.email}`}>
							<Mail className="h-7 w-7"></Mail>
						</a>
						<a href={`tel:${match.phone}`}>
							<PhoneCall className="h-7 w-7"></PhoneCall>
						</a>
					</div>
				</div>
				{/* Chat messages area with background image. */}
				<div className="flex-grow overflow-y-auto mb-4 rounded-3xl bg-white shadow p-4">
					<div className="flex-grow w-full h-full bg-[url('../../client/public/CollegeBuddy-Chat-bg.png')] bg-contain rounded-xl shadow p-4">
